 ...
 ?>
 <div class="container">
 	<h3 class="center">Ubah Data Gaji</h3>
 		...
 		<div class="card-panel">
 				...
 				<form class="col s12" method="post" enctype="multipart/form-data" >
 					<div class="row">
 						...
 						<div class="input-field col s6">
 							...
 							<input name="gaji_pokok" type="number" class="validate" required value="<?= $pecah['gaji_pokok']; ?>">
 					...
 					<div class="row">
 							...
 							<select name="id_ptkp" >
 								<?php foreach ($dataptkp as $key => $value): ?>
 									<option value="<?php echo $value['id_ptkp'] ?>"
 										<?php if ($pecah['id_ptkp']==$value["id_ptkp"]){echo "selected";} ?>><?php echo $value['nama_ptkp']?> <?php echo $value['keterangan_ptkp'];  ?></option>
 									<?php endforeach ?>
 								</select>
 								<label>PTKP</label>
 							</div>
 							<div class="input-field col s6">
 								...
 								<select  name="id_npwp">
 									<?php foreach ($datanpwp as $key => $value): ?>
 										<option  value="<?php echo $value['id_npwp'] ?>"
 											<?php if ($pecah["id_npwp"]==$value["id_npwp"]){echo "selected";} ?> ><?php echo $value['status_npwp'] ?>         
 										</option>
 									<?php endforeach ?>
 								</select>
 								<label>NPWP</label>
 						...
 						<div class="row">
 								...
 								<select  name="id_status">
 									<?php foreach ($datastatus as $key => $value): ?>
 										<option value="<?php echo $value['id_status']; ?>"
 											<?php if($pecah["id_status"]==$value["id_status"]){echo "selected";} ?> ><?php echo $value['nama_status'] ?>
 										</option>
 									<?php endforeach ?>
 								</select>
 							...
 							<div class="input-field col s6">    
 								...
 								<input type="date" name='tgl_masuk' value="<?= $pecah['tgl_masuk']; ?>" min="2020-01-01" max="2020-12-31" > 
          ...
          if ($id_status == 0) {
            ...
            $jp = 0;
 					...
 					$koneksi->query("UPDATE tbl_payroll SET 
 						...
 						alokasi_hari='$alokasi_hari',
 						alokasi_thr='$alokasi_thr',
 						persenan_thr='$persenan_thr',
 						thr='$thr',
 						bpjs_jkm='$bpjs_jkm',
 						bpjs_kes='$bpjs_kes',
 						gaji_kotor='$gaji_kotor',
 						bpjstk_jht='$bpjstk_jht',
 						bpjstk_jp='$bpjstk_jp',
 						...
 						pajak4='$pajak4',
 						pph21='$pph21',
 						bpjs_keset='$bpjs_keset',
 						gaji_bersih='$gaji_bersih',
 						id_status='$id_status',
 						jht='$jht',
 						jp='$jp'
 						WHERE id_payroll='$_GET[id]'") ;
